...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def generate_state_dict(
        ...
        optimizer_needs_model_template = generate_optimizer and base_metadata.get(
            "distrib_optim_sharding_type"
        ) != "dp_reshardable"
        if generate_model or optimizer_needs_model_template:
            # Optimizer formats defined in model space need this template even for optimizer-only loading.
    ...
    def load_checkpoint(
        ...
        supported_contents = {"model", "optimizer", "extra"}
        requested_contents = (
            set(self.checkpoint_load_contents) & supported_contents
            if load_contents is None
            else set(load_contents)
        )
        unknown_contents = requested_contents - supported_contents
        if unknown_contents:
            raise ValueError(f"Unknown checkpoint load contents: {sorted(unknown_contents)}")
        if not requested_contents.issubset(self.checkpoint_load_contents):
            raise ValueError(
                f"Requested checkpoint contents {sorted(requested_contents)} are not enabled by "
                f"load_contents={self.checkpoint_load_contents}"
            )
        load_model = "model" in requested_contents
        load_optimizer = "optimizer" in requested_contents
        load_extra = "extra" in requested_contents
        ...
        if sharded_sd_metadata is None:
            ...
            if load_content_metadata is not None:
        ...
        try:
            sharded_state_dict = self.generate_state_dict(
                load_model and self.use_dist_checkpointing,
                load_optimizer,
                load_extra,
                is_loading=True,
            ...
            log_with_rank(
                f"Generated state dict for loading: {sharded_state_dict.keys()}", rank=self.rank, logger=logger
            )
            if load_optimizer and stage_callback is not None:
                stage_callback("after_optimizer_template")
            ...
            if load_model and self.use_dist_checkpointing:
                ...
                for vpp_rank, model in enumerate(self.model):
                    key = "model" if len(self.model) == 1 else f"model{vpp_rank}"
            ...
            elif load_model and self.use_hf_checkpoint and self.peft_cls is None:
                ...
                log_with_rank(
                    f"Loaded HF model checkpoint from {hf_model_path} with bridge", rank=self.rank, logger=logger
                )

            if load_model and self.peft_cls is not None:
                ...
                if os.path.exists(adapter_ckpt_path):
                    ...
                    log_with_rank(
                        f"Loaded adapter checkpoint from {adapter_ckpt_path}", rank=self.rank, logger=logger
                    )
                else:
                    log_with_rank(
            ...
            if load_optimizer:
                assert "optimizer" in state_dict, (
                    f"Optimizer state dict not found in {state_dict.keys()}. "
                    f"Please check the checkpoint file {local_path}."
                )
                ...
                restore_lr_scheduler = self.lr_scheduler is not None and (
                    self.use_checkpoint_opt_param_scheduler or load_contents is not None
                )
                if restore_lr_scheduler:
                    assert "lr_scheduler" in state_dict, (
                        f"LR scheduler state dict not found in {state_dict.keys()}. "
                        f"Please check the checkpoint file {local_path}."
                    )
                    lr_scheduler_state_dict = state_dict["lr_scheduler"]
                    self.lr_scheduler.load_state_dict(lr_scheduler_state_dict)
                    log_with_rank(
                        f"Loaded LR scheduler checkpoint from {local_path}", rank=self.rank, logger=logger
                    )
            ...
            if state_dict is not None:
                state_dict.clear()
            if sharded_state_dict is not None and sharded_state_dict is not state_dict:
                sharded_state_dict.clear()
            model_state_dict = None
            optimizer_state_dict = None
            lr_scheduler_state_dict = None
            rng_state = None
